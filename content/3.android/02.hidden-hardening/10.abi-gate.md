...
`libhiddendecrypt.so` 按方法名和 JNI 签名调用加密 dex。如果改了 `HiddenLandingPage` 的方法表，却仍使用旧 so（或反过来），`GetMethodID` 会静默失败，表现为 init 成功，但所有 invoke 都是空操作。因此，加载末尾加了一道 **整数 ABI** 检查。
...
未初始化时，Java fallback 的 op 16 也返回 `12`，避免调试接口在加载前报一个「假版本」。
