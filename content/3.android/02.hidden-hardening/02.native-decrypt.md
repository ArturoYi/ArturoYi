---
title: 安卓代码加固（2）：Native解密
description: libhiddendecrypt.so 自实现 AES-256-GCM。Java 不接触明文 DEX，用完立刻清零。
date: 2026-09-21
navigation:
  icon: i-lucide-cpu
seo:
  title: 安卓代码加固（2）：Native AES-256-GCM 解密 Hidden DEX
...
`aes256_gcm.c` 自带 S-box、密钥扩展、GHASH、GCTR。不链接系统 crypto，可以减少通过 hook 常见 JNI 或 OpenSSL 符号截取明文的路径。代价是必须自行保证实现正确。构建期由 `PayloadEncryptor` 先加密再解密核对。
:::
